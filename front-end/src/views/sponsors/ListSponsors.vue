<template>
  <section class="page-section">
    <b-container>
     <HeaderPage title="Gestão de Sponsor"/>
          ...
          <table class="table table-striped">
            ...
            <tbody>
              <tr v-for="sponsor of sponsors" :key="sponsor._id">                
                ...
                <td class="pt-4">{{sponsor.type==="admin"?"Administrador":"Utilizador normal"}}</td>
  ...
  methods: {
    ...
    viewSponsor(id) {
      ...
      this.$fire({
        ...
        imageUrl: require(`@/assets/avatars/${this.getUserLevelByPoints(sponsor.gamification.points).avatar}.png`),
        imageWidth: 150,
        imageHeight: 150,
    ...
    generateTemplate(sponsor) {
      return `
          <h5>${sponsor.gamification.points} pontos (${this.getUserLevelByPoints(sponsor.gamification.points).name})</h5>
          <p>${sponsor.description}</p>
          <p>
          <b>Nome:</b> ${sponsor.name} <br>
          <b>Tipo de utilizador:</b> ${sponsor.type==="admin"?"Administrador":"Utilizador normal"} <br>
